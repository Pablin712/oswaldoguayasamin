<x-app-layout>
    @canany(['gestionar horarios', 'ver horarios', 'ver horarios por aula'])
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Horario del Aula') }}: {{ $aula->nombre }}
            </h2>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Aula</p>
                            <p class="text-base font-medium text-gray-900 dark:text-gray-100">{{ $aula->nombre }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Capacidad</p>
                            <p class="text-base font-medium text-gray-900 dark:text-gray-100">{{ $aula->capacidad ?? 'No especificada' }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-600 dark:text-gray-400">Tipo</p>
                            <p class="text-base font-medium text-gray-900 dark:text-gray-100">{{ $aula->tipo ?? 'Estándar' }}</p>
                        </div>
                        <div>
                                ...
                                <tr>
                                    ...
                                    @foreach(['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'] as $dia)
                                        <td class="px-3 py-3 text-sm text-gray-900 dark:text-gray-100">
                                            ...
                                            @elseif($clase)
                                                <div class="bg-purple-100 dark:bg-purple-900 border border-purple-300 dark:border-purple-700 rounded-lg p-3">
                                                    <p class="font-semibold text-sm text-purple-900 dark:text-purple-100">
                                            ...
                                            @else
                                                <div class="h-16 bg-green-50 dark:bg-green-950 rounded">
                                                    <p class="text-center text-xs text-green-600 dark:text-green-400 pt-6">Disponible</p>
                                                </div>
                                            @endif
                                        </td>
                                    @endforeach
