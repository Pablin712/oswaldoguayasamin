...
<x-modal name="delete-area" maxWidth="md" focusable>
    <div x-data="{
        areaId: null,
        areaNombre: '',
        openDelete(id, nombre) {
            ...
            $dispatch('open-modal', 'delete-area');
        }
    }"
    @open-delete-modal.window="openDelete($event.detail.id, $event.detail.nombre)"
    class="p-6">
        <form method="POST" :action="`{{ route('areas.index') }}/${areaId}`" x-ref="deleteForm">
            @csrf
            @method('DELETE')

            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                {{ __('¿Estás seguro de eliminar esta área?') }}
            </h2>

            <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
                {{ __('Estás a punto de eliminar el área:') }}
                <span class="font-semibold text-gray-900 dark:text-gray-100" x-text="areaNombre"></span>.
                {{ __('Esta acción no se puede deshacer.') }}
            </p>

            <div class="flex justify-end gap-3">
                <button type="button" @click="$dispatch('close-modal', 'delete-area')"
